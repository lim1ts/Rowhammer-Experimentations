...
class BitFlipper {
  static const int hammer_count = 1000000;
...
class ProcessLauncher {
  int parent_socket;
	//The parent wants to receive data from child process.
	// Parent process closes fd[1]
	// Child process closes fd[0]
	// On a technical note, the EOF will never be returned if the unnecessary ends
	// are not explicitly closed

  //Child process closes the input side of pipe. 
	// then reads childsocket into buf
	// 
...
public:
  ProcessLauncher() {
    int sockets[2];
    //Creates a pair of connected sockets of SOCK_STREAM type.
	// AF_UNIX = local communication
	// SOCK_STREAM = two way connection based byte streams.
	// Protocol 0 = IPv6 Hop-by-Hop Option (?)
