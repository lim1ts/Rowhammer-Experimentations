...
uint32_t bitmaskLowerBits = (1 << 22) -1;

uint32_t *pick_addr(int bankCount) {
  size_t offset = (rand() << 12) % mem_size; 
  uint32_t *randomAddr;
  uint32_t lowerBits;
  int bits[21];
  int bankNumbers = bankCount;
  int bankBits[3];

  for ( int i = 0 ; i < 3 ; i ++ ) {
	bankBits[i] = bankNumbers&1;
	bankNumbers >>=1;
  }
  ...
  while(!isSameBank){
  	...
  	offset = (rand() << 12) % mem_size; 
	randomAddr = (uint32_t *) (g_mem + offset);
	//printf ("random address: %lx\n ", (void *) randomAddr);
	//check bank bits.
	
	lowerBits =  (uint32_t ) randomAddr & bitmaskLowerBits;
	...
	if (bits[13]^bits[17] == bankBits[0] && 
		bits[14]^bits[18] == bankBits[1] &&
		bits[16]^bits[20] == bankBits[2]) {
		isSameBank = true;
	}
  }
  return randomAddr;
...
static void pick_addrs(struct OuterSet *set) {
  int randomBank = rand() % 8; 
  for (int j = 0; j < ITERATIONS; j++) {
    for (int a = 0; a < ADDR_COUNT; a++) {
      set->inner[j].addrs[a] = (uint32_t *) pick_addr(randomBank);
    }
  }
}
...
static bool check(struct BitFlipInfo *result) {
  ...
  for (ptr = (uint64_t *) g_mem; ptr < end; ptr++) {
    ...
    if (got != expected) {
      printf("error at %p (phys N/A" PRIx64 "): got 0x%" PRIx64 "\n",
             ptr, got);
      found_error = true;
      g_errors_found++;
    ...
    for (int idx2 = idx1 + 1; idx2 < ADDR_COUNT; idx2++) {
      ...
      if (check(&bit_flip_info)) {
        ...
        printf("RESULT PAIR,0x%" PRIx64 ",0x%" PRIx64 ",0x%" PRIx64 ",%i,%i\n",
                addr1,
                addr2,
                bit_flip_info.victim_virtual_addr,
...
bool narrow_down(struct OuterSet *outer) {
  ...
  for (int j = 0; j < ITERATIONS; j++) {
    ...
    if (check(NULL)) {
      ...
      for (int a = 0; a < ADDR_COUNT; a++) {
        printf("  logical=%p, physical=n/a" PRIx64 "\n",
               inner->addrs[a]);
      }
      found = true;
