...
//MODIFIED TO LET ME RUN FROM DESKTOP.
const char target_prog_path[] = "./Downloads/36310/out/target_prog";
...
uint64_t get_physical_addr(uintptr_t virtual_addr) {
  ...
  uint64_t phys =  (frame_num * page_size) | (virtual_addr & (page_size - 1));

  printf("frame_num = %llx\n", (long long) frame_num);
  printf("value = %llx\n", (long long) value);
  printf("Virtual address = 0x%llx\n", (long long) virtual_addr);
  printf("Physical addresss = 0x%llx\n", (long long) phys);
